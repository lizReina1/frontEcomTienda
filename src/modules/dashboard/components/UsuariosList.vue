<template>
  <div  id="UsuarioList">
    ...
    <!-- <div class="col-sm-4">
        <button type="button" class="btn btn-info mb-3" @click="$emit('create')">Crear Usuario</button>
    </div> -->
    <div class="col">
    <table class="table">
      <thead>
...
export default {
 ...
 methods: {
  filteredUsuarios() {
      if (!this.searchDate) return this.usuarios;
      const searchDate = new Date(this.searchDate);
      return this.usuarios.filter(usuario => {
        const usuarioDate = new Date(usuario.date);
        return usuarioDate.toDateString() === searchDate.toDateString();
      });
    },
},
computed: {
