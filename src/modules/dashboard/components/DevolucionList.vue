                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(refund, index) in paginatedRefunds" :key="refund.id">
                            ...
                            <td>{{ customerNames[refund.customer_id] || 'Cargando...' }}</td>
                            <td>
                                <div class="row">
                                    <div class="col-sm-2" @click="$emit('view', refund)">
                                    ...
                                    <div class="col-sm-2" @click="$emit('delete', refund.id)">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            ...
                            <ul class="pagination flex-wrap d-flex justify-content-center">
                                <li class="page-item">
                                    <button class="page-link" @click="currentPage--" :disabled="currentPage === 1"
                                        data-abc="true"><i class="fa fa-angle-left"></i> Anterior</button>
                                </li>
                                ...
                                <li class="page-item">
                                    <button class="page-link" @click="currentPage++"
                                        :disabled="currentPage === totalPages" data-abc="true">Siguiente <i
...
export default {
    ...
    methods: {
        filteredDevoluciones() {
            if (!this.searchDate) return this.refunds;
            const searchDate = new Date(this.searchDate);
            return this.refunds.filter(refunds => {
                const refundDate = new Date(refunds.date);
                return refundDate.toDateString() === searchDate.toDateString();
            });
        },
        async loadCustomerNames() {
            if (!this.refunds || this.refunds.length === 0) return;

            const customerIds = [...new Set(this.refunds
                .filter(refund => refund && refund.customer_id)
                .map(refund => refund.customer_id))];

            for (const id of customerIds) {
                if (!this.customerNames[id]) {
                    await this.fetchCustomerName(id);
                }
            }
        },
        async fetchCustomerName(customerId) {
            try {
    ...
    computed: {
        paginatedRefunds() {
            ...
            return this.filteredDevoluciones().slice(startIndex, endIndex);
        },
        totalPages() {
            return Math.ceil(this.filteredDevoluciones().length / this.pageSize);
        },
        visiblePageNumbers() {
